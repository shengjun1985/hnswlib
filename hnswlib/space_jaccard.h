...
#define fun_u64 accu_num += __builtin_popcountl(a[i] & b[i]); accu_den += __builtin_popcountl(a[i] | b[i])
...
float jaccard(const void *pVect1, const void *pVect2, const void *qty_ptr) {
    return bvec_jaccard((const uint8_t*)pVect1, (const uint8_t*)pVect2, *(size_t*)qty_ptr);
}
